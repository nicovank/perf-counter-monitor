...
std::uint64_t sum_over_dims(const std::vector<std::uint64_t> &data,
  ...
  return std::accumulate(
      dims.begin(), dims.end(), std::uint64_t(0),
      [&data](std::uint64_t sum, std::size_t dim) { return sum + data[dim]; });
}
...
ftxui::Element plot_ratio(const std::deque<std::vector<std::uint64_t>> &data,
  ...
  for (std::size_t i = 0; i < data.size(); ++i) {
    const auto numerator = static_cast<double>(sum_over_dims(data[i], xDims));
    const auto denominator = static_cast<double>(sum_over_dims(data[i], yDims));

    const auto value = denominator == 0 ? 0 : (numerator / denominator * 100.0);
...
int main(int argc, char **argv) {
  ...
  std::cout << "Monitoring " << nameByEvent.at(event) << " every " << period_ms
